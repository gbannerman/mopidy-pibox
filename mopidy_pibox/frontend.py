...
class PiboxFrontend(pykka.ThreadingActor, core.CoreListener):
    ...
    def play_song_from_playlist(self):
        if self.config["offline"]:
            ...
            playlist = self.core.playlists.get_items(
                self.config["default_playlist"]
            ).get()
        else:
        ...
        for ref in playlist:
            ...
            if not (self.played_already(new_track_uri)):
                self.core.tracklist.add(uris=[new_track_uri], at_position=0).get()
                self.logger.info("Auto-added " + ref.name + " to tracklist")
                if self.core.playback.get_state().get() == core.PlaybackState.STOPPED:
                    self.core.playback.play()
                break
